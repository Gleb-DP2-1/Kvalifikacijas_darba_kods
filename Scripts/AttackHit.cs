...
public class AttackHit : MonoBehaviour
{
    ...
    public bool isExploderAttackHit = false;
    [SerializeField] public float hitPower = 1;
    public int hero_dmg;
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        {
            if (col.GetComponent<RealMovement>() != null)
            {

                RealMovement.Instance.GetHurt(targetSide, hitPower, isBomb);
                if (isExploderAttackHit)
                {
                    transform.parent.GetComponent<RealEnemyBase>().Die();
        ...
        //Attack Enemies
        else if (attacksWhat == AttacksWhat.EnemyBase && col.GetComponent<RealEnemyBase>() != null)
        {
            col.GetComponent<RealEnemyBase>().GetHurt(targetSide, hero_dmg);
        }

        //Attack Breakables
        else if (attacksWhat == AttacksWhat.EnemyBase && col.GetComponent<RealEnemyBase>() == null && col.GetComponent<Breakable>() != null)
        {
    ...
    private void Update()
    {
        hero_dmg = RealMovement.Instance.dmg;
    }
}
